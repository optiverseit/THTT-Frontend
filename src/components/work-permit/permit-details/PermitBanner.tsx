...
import { Clock, Shield, TrendingUp, Users, Share2, Printer, MessageCircle, Check, Link2 } from "lucide-react";
import {
  shareToPlatform,
  triggerNativeShare,
  openSharePopup,
...
const PermitBanner: React.FC<PermitBannerProps> = ({ id, country }) => {
  ...
  const shareTitle = `${country.name} Work Permit | Trip Himalaya Tours & Travel`;
  const shareText = `Check out work permit process, requirements and fees for ${country.name} on Trip Himalaya Tours & Travel!`;
  ...
  const handlePrint = () => {
    ...
    document.title = `${country.name} - Work Permit - Trip Himalaya`;
    window.print();
  ...
  const handleWhatsApp = () => {
    const msg = encodeURIComponent(
      `Hello Trip Himalaya (Work Permit Team)! I am interested in the work permit process for ${country.name}. Please guide me through the requirements and next steps.`
    );
  ...
  const shareButtons = [
    {
      ...
      bg: "#1877F2",
      svg: <svg viewBox="0 0 24 24" fill="white" width="15" height="15"><path d="M18 2h-3a5 5 0 00-5 5v3H7v4h3v8h4v-8h3l1-4h-4V7a1 1 0 011-1h3z"/></svg>,
    },
    {
      name: "Instagram",
      action: async () => {
      ...
      svg: <svg viewBox="0 0 24 24" fill="white" width="14" height="14"><rect x="2" y="2" width="20" height="20" rx="5" ry="5" fill="none" stroke="white" strokeWidth="2"/><path d="M16 11.37A4 4 0 1112.63 8 4 4 0 0116 11.37z" fill="white"/><circle cx="17.5" cy="6.5" r="1.5" fill="white"/></svg>,
    },
    {
      name: "LinkedIn",
      action: () => shareToPlatform("linkedin", shareData),
      bg: "#0A66C2",
      svg: <svg viewBox="0 0 24 24" fill="white" width="14" height="14"><path d="M16 8a6 6 0 016 6v7h-4v-7a2 2 0 00-2-2 2 2 0 00-2 2v7h-4v-7a6 6 0 016-6zM2 9h4v12H2z"/><circle cx="4" cy="4" r="2" fill="white"/></svg>,
    },
    {
      name: "Twitter / X",
      action: () => shareToPlatform("twitter", shareData),
      bg: "#000000",
      svg: <svg viewBox="0 0 24 24" fill="white" width="13" height="13"><path d="M18.244 2.25h3.308l-7.227 8.26 8.502 11.24H16.17l-5.214-6.817L4.99 21.75H1.68l7.73-8.835L1.254 2.25H8.08l4.713 6.231zm-1.161 17.52h1.833L7.084 4.126H5.117z"/></svg>,
    },
  ];
  ...
  return (
    <div className="relative min-h-[22rem] sm:min-h-[20rem] md:h-80 w-full overflow-hidden shadow-md flex flex-col justify-center">
      ...
      <div className="relative z-10 w-full px-4 sm:px-8 md:px-12 lg:px-16 xl:px-20 pt-12 sm:pt-14 pb-6 sm:pb-8 flex flex-col justify-between h-full">
        ...
        {/* Top row: title + action buttons — matching visa service layout */}
        <div className="flex flex-col md:flex-row md:items-center justify-between gap-4">
          ...
          {/* Action buttons — matching visa service style exactly */}
        ...
        {/* ── SHARE BUTTON — top-right of hero (matching visa service style) ── */}
        <div ref={shareRef} className="print:hidden absolute top-12 right-4 sm:top-14 sm:right-8 md:right-12 lg:right-16 xl:right-20 z-20">
          {/* Share popup */}
          {isShareOpen && (
